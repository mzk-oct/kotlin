...
typealias S = Long
typealias F = Long
abstract class LazySegmentTree(val size: Int, initial: IntArray) {
    ...
    fun update(position: Int, func: F) {
        val pos = position + (1 shl log)
        for (i in log downTo 1) {
            applyLazy(pos shr i)
        }
    ...
    fun update(from: Int, until: Int, func: F) {
    ...
    fun searchRight(left: Int, predicate: (S) -> Boolean): Int? {
    ...
    fun searchLeft(right: Int, predicate: (S) -> Boolean): Int? {
        if (!predicate(es)) return null
        if (right == 0) return 0
    ...
    protected abstract val es: S
    protected abstract val ef: F
    ...
    private fun appendLazy(position: Int, func: F) {
        if (position < 1 shl log) {
            lazy[position] = composite(func, lazy[position])
        }
        vec[position] = map(vec[position], func)
    }
    abstract fun map(value: S, func: F): S
    abstract fun plus(left: S, right: S): S
    abstract fun composite(new: F, old: F): F
    companion object {
