...
class LazyAddMaxSeg private constructor(val size: Int, private val vec: LongArray) {
    ...
    fun get(from: Int, until: Int): Long {
        ...
        while (l < r) {
            if (l and 1 == 1) {
                result = maxOf(result, vec[l] + lazy[l])
                ++l
            }
            if (r and 1 == 1) {
                --r
                result = maxOf(result, vec[r] + lazy[r])
            }
            l = l shr 1
            r = r shr 1
        }
    ...
    private tailrec fun update(position: Int) {
        ...
        vec[position] = maxOf(vec[c] + lazy[c], vec[c + 1] + lazy[c + 1])
