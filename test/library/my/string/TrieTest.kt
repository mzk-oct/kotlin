...
internal class TrieTest {
    ...
    fun test(data: Pair<String, List<String>>) {
        ...
        val actual = countTrie(sentence, dictionary)
        assert(actual.contentEquals(expected)){
            "expected: ${expected.joinToString(", ", "[", "]")}\nactual: ${actual.joinToString(", ", "[", "]")}"
        }
    }
    fun countTrie(sentence: String, dictionary: List<String>): IntArray {
        val trie = Trie.make(dictionary.toTypedArray())
        val result = IntArray(dictionary.size)
        for (idx in sentence.indices) {
            for (i in trie.matchAll(sentence, idx)) {
                ++result[i]
            }
        }
        return result
